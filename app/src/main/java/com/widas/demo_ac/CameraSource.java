...
public class CameraSource {
  ...
  public CameraSource(Activity activity, ICameraSource iCameraSource) {
    this.activity = activity;

    processingRunnable = new FrameProcessingRunnable();

    this.iCameraSource = iCameraSource;
    ...
    public void run() {
      ...
      while (true) {
        synchronized (lock) {
          while (active && (pendingFrameData == null)) {
            try {
             ...
             // Logger.addRecordToLog(TAG + "Frame processing loop terminated err "+ e);
             Log.d("CameraSource",TAG + "Frame processing loop terminated err "+ e);
              return;
            }
          }
        ...
        try {
          ...
          Log.d("CameraSource", "Exception thrown from receiver err "+ t.getLocalizedMessage());
        } finally {
  ...
  public void captureImage(){
    camera.takePicture(null, null, new Camera.PictureCallback() {
      ...
      public void onPictureTaken(byte[] data, Camera camera) {
        ...
        iCameraSource.pictureTaken(file);
        stop();
  ...
  public File saveImage(byte[] bytes) {
    ...
    try {
      File f = CameraUtils.getFaceRecgFile(activity);
      ...
      fo.close();
      Timber.d( "File Saved::---> %s ", f.getAbsolutePath());

      return f;
    } catch (IOException e1) {
      Log.d("CameraSource","saveImage  IOException "+e1.getLocalizedMessage());
    }
    return null;
